...
function getRecipe() {
  ...
  fetch(`/api/recipes/${recipeId}`)
    ...
    .catch(err => {
      console.log(err);
      alert('Cannot find a recipe with this id! Taking you back.');
      window.history.back();
    });
}
...
function printRecipe(recipeData) {
  ...
  const { recipeName, createdBy, createdAt, size, ingredients, comments } = recipeData;

  $recipeName.textContent = recipeName;
  $createdBy.textContent = createdBy;
  $createdAt.textContent = createdAt;
  $size.textContent = size;
  $ingredientsList.innerHTML = ingredients
    .map(ingredient => `<span class="col-auto m-2 text-center btn">${ingredient}</span>`)
    .join('');
...
function printComment(comment) {
  ...
  commentDiv.classList.add('my-2', 'card', 'p-2', 'w-100', 'text-primary', 'rounded');

  const commentContent = `
      <h5 class="text-light">${comment.writtenBy} commented on ${comment.createdAt}:</h5>
...
function printReply(reply) {
  return `
  <div class="card p-2 rounded bg-secondary text-dark">
...
function handleNewReplySubmit(event) {
  ...
  fetch(`/api/comments/${recipeId}/${commentId}`,{
    method: 'PUT',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json'
